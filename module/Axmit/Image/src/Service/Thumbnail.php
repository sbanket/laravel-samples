 ...
 * @method \Intervention\Image\Image trim(string $base = 'top-left', array $away, integer $tolerance = 0, integer $feather = 0) Trim away image space in given color. Define an optional base to pick a color at a certain position and borders that should be trimmed away. You can also set an optional tolerance level, to trim similar colors and add a feathering border around the trimed image.
...
class Thumbnail
{
     ...
     *
     * @param Cloud  $filesystem
     * @param Image  $image
     * @param string $path
     */
    public function __construct(Cloud $filesystem, Image $image, string $path)
    {
     ...
     * @return string
     */
    public function save($name = null, $quality = 90)
    {
        $filename = $name ? $this->generateName($name) : $this->getName();
        $path     = sprintf('%s/%s', $this->path, $filename);

        $image = $this->image->encode($this->getExtension(), $quality);

        $this->filesystem->put($path, $image->encoded, Filesystem::VISIBILITY_PUBLIC);
     ...
     * @return string
     */
    protected function generateName($name = null)
    {
        return $this->name = sprintf('%s.%s', $name ?: Str::random(), $this->getExtension());
    }
}
